...
## 문제 해석
원점과의 거리가 d를 넘지 않는 점 x, y의 위치를 구해야 한다.
...
a가 0부터 d/k 까지 for문을 돌면서 가능한 y의 개수를 구하면 되는데,
x 값이 정해져 있다면, y의 최대값은 sqrt(d^2 - x^2) 이다.
y의 개수는 0부터 최대값의 자연수이므로, sqrt(d^2-x^2).toInt()+1 이다.
